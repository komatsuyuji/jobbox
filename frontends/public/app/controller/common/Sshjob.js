...
Ext.define('Jobbox.controller.common.Sshjob', {
  ...
  onLoadRecord: function(record) {
    ...
    Jobbox.model.Jobunit.load(record.data.id, {
      success: function(rec) {
        var rec_sshjob = rec['Jobbox.model.SshjobHasOneInstance'];
        if (rec_sshjob) {
          rec_sshjob.getProxy().url = location.pathname + '/jobunits/' + rec.data.id + '/sshjobs';
          JobboxSetForm(form_sshjob, rec_sshjob);
        }
        var store_error = JobboxGetCondition(rec, CONDITION_METHOD_ERROR, 'jobunits');
        var store_branch = JobboxGetCondition(rec, CONDITION_METHOD_BRANCH, 'jobunits');
  ...
  onLoadProcRecord: function(record) {
    ...
    JobboxSetForm(form_jobunit, record);
    JobboxSetForm(form_sshjob, record['Jobbox.model.SshjobHasOneInstance']);

    var store_error = JobboxGetCondition(record, CONDITION_METHOD_ERROR, 'proc_jobunits');
    var store_branch = JobboxGetCondition(record, CONDITION_METHOD_BRANCH, 'proc_jobunits');
    grid_error.reconfigure(store_error);
    grid_branch.reconfigure(store_branch);
  },
  ...
  onLoadHistRecord: function(record) {
    ...
    var store_error = JobboxGetCondition(record, CONDITION_METHOD_ERROR, 'hist_jobunits');
    var store_branch = JobboxGetCondition(record, CONDITION_METHOD_BRANCH, 'hist_jobunits');
    grid_error.reconfigure(store_error);
    grid_branch.reconfigure(store_branch);
  },
