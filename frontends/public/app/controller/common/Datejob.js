...
Ext.define('Jobbox.controller.common.Datejob', {
  ...
  onLoadRecord: function(record) {
    ...
    Jobbox.model.Jobunit.load(record.data.id, {
      success: function(rec) {
        var rec_datejob = rec['Jobbox.model.DatejobHasOneInstance'];
        if (rec_datejob) {
          rec_datejob.getProxy().url = location.pathname + '/jobunits/' + rec.data.id + '/datejobs';
          JobboxSetForm(form_datejob, rec_datejob);
        }
        var store_datecond = rec['jobbox.model.datecondsStore'];
        if (store_datecond) {
  ...
  onLoadProcRecord: function(record) {
    ...
    JobboxSetForm(form_jobunit, record);
    JobboxSetForm(form_datejob, record['Jobbox.model.DatejobHasOneInstance']);
    if (record['jobbox.model.datecondsStore']) {
      grid_datecond.reconfigure(record['jobbox.model.datecondsStore']);
    }
  },
