...
Ext.define('Jobbox.controller.Calendar', {
  ...
  onCellclick: function(grid, td, cellIndex, record, tr, rowIndex, e) {
    ...
    field.setValue(JOBBOX_THIS_YEAR);
    ...
    Jobbox.model.Calendar.load(record.data.id, {
      failure: function(record, operation) {},
      success: function(rec, operation) {
        panel.setDisabled(false);
        form.loadRecord(rec);
        if (rec['jobbox.model.calendardatesStore']) {
          rec['jobbox.model.calendardatesStore'].getProxy().url = location.pathname + '/calendars/' + record.data.id + '/calendar_dates';
  ...
  onAddCalendar: function(button, e) {
    ...
    var record = new Jobbox.model.Calendar();
    record['jobbox.model.calendardatesStore'] = new Ext.data.Store({
      model: 'Jobbox.model.CalendarDate',
    });
        ...
        if (btn == "yes") {
          ...
          store.sync({
            success: function(records, operation) {
              ...
              panel.setDisabled(true);
              form.getForm().reset(true);
              var combo_store = Jobbox.app.getStore('CalendarCombo');
              combo_store.reload();
              me.onLoadCalendarDate();
            }
          });
    ...
    record.save({
      success: function(rec, operation) {
        if (!rec.data.id) {
          ...
          rec.setId(data.id);
          rec['jobbox.model.calendardatesStore'].getProxy().url = location.pathname + '/calendars/' + rec.data.id + '/calendar_dates';
        }
        rec['jobbox.model.calendardatesStore'].sync();
  ...
  onClearCalendar: function(button, e) {
    ...
    var form = me.getCalendarEdit();
    var record = form.getRecord();
    if (record['jobbox.model.calendardatesStore']) {
      record['jobbox.model.calendardatesStore'].removeAll();
    }
    me.onLoadCalendarDate();
  },
  ...
  onSelectDate: function(datepicker, date, eOpts) {
    var ctrl = Jobbox.app.getController('Calendar');
    var form = ctrl.getCalendarEdit();
    var record = form.getRecord();
    var store = record['jobbox.model.calendardatesStore'];
  ...
  onLoadCalendarDate: function() {
    ...
    var store = null;
    if (record)
      store = record['jobbox.model.calendardatesStore'];
