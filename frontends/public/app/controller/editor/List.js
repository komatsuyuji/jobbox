...
Ext.define('Jobbox.controller.editor.List', {
  ...
  onCopyJobunit: function(button, e) {
    ...
    jobbox_selected_jobunits.push(record);
  },
  ...
  onPasteJobunit: function(button, e) {
    ...
    var parent_node = store.getNodeById(jobbox_selected_parent.data.id);
    if (!parent_node)
      return;
    ...
    jobbox_selected_jobunits.forEach(function(record) {
      ...
      if (jobbox_selected_parent.data.id == 0 && record.data.kind >= JOBUNIT_KIND_ROOTJOBNET) {
        cnt++;
        Ext.Msg.alert(I18n.t('views.msg.error'), I18n.t('views.msg.create_rootjobnet'));
        return;
      }
