...
Ext.define('Jobbox.controller.Header', {
  extend: 'Ext.app.Controller',
  refs: [{
    ref: 'jobboxHeader',
    selector: 'jobboxHeader'
  }, {
    ref: 'userProfile',
  ...
  init: function() {
    this.control({
      ...
      'jobboxHeader displayfield[itemId=tf_username]': {
        afterrender: this.onAfterrenderUser,
      },
      'jobboxHeader button[action=logout]': {
        click: this.onLogout,
      },
    });
  },
  ...
  onAfterrender: function(panel) {
    ...
    field.setValue(jobbox_login_user.data.name);
  },
  ...
  onLogout: function(button, e) {
    var session = new Jobbox.model.Session();
    session.setId(1);
    session.destroy({
      callback: function() {
