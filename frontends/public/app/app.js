...
Ext.application({
  ...
  launch: function() {
    jobbox_login_user = null;
    Jobbox.model.Session.load(1, {
      failure: function(record, operation) {
        Ext.create('Jobbox.view.session.Win').show();
      },
      success: function(record, operation) {
        jobbox_login_user = new Jobbox.model.User(record.getData());
        if (location.search.indexOf('proc_jobunit_id') == 1) {
          Ext.create('Jobbox.view.process.Viewport');
        } else {
          Ext.create('Jobbox.view.user.Profile');
          Ext.create('Jobbox.view.Viewport');
        }
      }
    });
  },
