...
int monitor_jobnet(jobunit_t * obj)
{
    ...
    jbxlog_trace("In %s() id: %llu, kind: %d, status: %d", __func__,
...
int monitor_jobs(void)
{
    ...
    rc_db =
        ...
        ("UPDATE proc_jobunits SET status = %d, start_time = NULL, end_time = NULL, timeout_occur = false, force_start = false, force_stop = false, pid = 0, updated_at = '%s' \
         ...
         JOBUNIT_STATUS_READY, jbx_time(), JOBUNIT_STATUS_ERROR, JOBUNIT_KIND_STARTJOB);
    if (rc_db != 0)
        return -1;
...
int monitor_background(jobunit_t * obj)
{
    ...
    jbxlog_trace("In %s() id: %llu, kind: %d, pid: %d, force_stop: %d",
    ...
    if (obj->pid == 0) {
        if (obj->force_stop == 0) {
            ...
            jbxlog_info("Force stop the job. name: %s, id: %llu, pid: %d",
    ...
    // check pid
    if (kill(obj->pid, 0) != 0) {
        jbxlog_warn("In %s() pid is not existed. id: %llu, pid: %d",
    ...
    if (kill(obj->pid, SIGTERM) != 0) {
        jbxlog_error("In %s() kill failed. id: %llu, pid: %d", __func__,
                     obj->id, obj->pid);
