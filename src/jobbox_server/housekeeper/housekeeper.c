...
int housekeeper_forcestart(void)
{
    ...
    rc = jbxdb_execute
        ("UPDATE proc_topjobnets SET status = %d, force_start = false, start_time = NULL, end_time = NULL, updated_at = '%s' WHERE status in (%d, %d) AND force_start = true",
         JOBUNIT_STATUS_READY, jbx_time(), JOBUNIT_STATUS_OUTSCHEDULE,
...
int housekeeper_topjobnet(jobunit_t * obj)
{
    ...
    jbxlog_debug("In %s() proc_topjobnet_id: %llu, proc_jobunit_id: %llu",
                       ...
                       const apr_uint64_t hist_jobnet_id)
{
    ...
    jbxlog_debug("In %s() proc_jobnet_id: %llu , hist_jobnet_id: %llu",
                 __func__, proc_jobnet_id, hist_jobnet_id);

    result =
        jbxdb_select("SELECT * FROM proc_jobunits WHERE parent_id = %llu",
                     proc_jobnet_id);
    if (result == NULL)
        return -1;
...
int housekeeper_history(void)
{
    ...
    history_period = jbx_time2str(hp);
    jbxlog_debug("In %s() history_period: %s", __func__, history_period);

    result =
        jbxdb_select
        ("SELECT id, kind FROM hist_jobunits WHERE created_at <= '%s'",
         history_period);
    jbx_free(history_period);
    if (result == NULL)
        return -1;
    ...
    while (dbi_result_next_row(result)) {
        ...
        jbxlog_debug("In %s() id: %llu, kind %d", __func__, id, kind);
