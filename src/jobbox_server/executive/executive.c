...
int executive_exec(void)
{
    ...
    jbxlog_debug("In %s() num: %d", __func__, num);
...
int executive_run(jobunit_t * obj)
{
    ...
    case JOBUNIT_KIND_WINJOB:
        ...
        break;
    default:
        jbxlog_error("In %s() unknown job kind. id: %llu, kind: %d",
...
{
    jbxlog_trace("In %s() id: %llu, name: %s, kind: %d", __func__,
...
int executive_background(jobunit_t * obj)
{
    ...
    pid = jbx_fork();
    if (pid == -1) {
        jbxlog_error("In %s() the first fork failed", __func__);
        return -1;
    } else if (pid > 0) {
        ec = -1;
        wait(&ec);
        if (WEXITSTATUS(ec) != 0) {
            jbxlog_error("In %s() the second fork failed. exitcode: %d",
    ...
    setsid();
    jbxlog_debug("In %s() the first fork is successfully", __func__);
    ...
    jbxlog_debug("In %s() the second fork is successfully", __func__);
    ...
    // set pid
    rc_db =
        jbxdb_execute("UPDATE proc_jobunits SET pid = %d WHERE id = %llu",
                      getpid(), obj->id);
    if (rc_db != 0)
        goto error;
    ...
    case JOBUNIT_KIND_WINJOB:
        ...
        break;
    default:
        jbxlog_error("In %s() unknown the job kind '%d'", __func__,
                     obj->kind);
        break;
    }

    // check the result
    jbxlog_debug("In %s() the job return code: %d, id: %llu", __func__,
                 rc_job, obj->id);
    jbxdb_begin();
