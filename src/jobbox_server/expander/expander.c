...
int expander_topjobnet(jobunit_t * obj)
{
    ...
    rc_db =
        ...
        ("UPDATE proc_topjobnets SET status = %d, proc_jobunit_id = %llu, path = '%s', updated_at = '%s' WHERE id = %llu",
         JOBUNIT_STATUS_STANDBY, proc_jobunit_id, obj->path, jbx_time(),
         proc_topjobnet_id);
    if (rc_db != 0)
        return -1;
                    ...
                    const char *jobnet_path)
{
    ...
    result =
        jbxdb_select("SELECT * FROM jobunits WHERE parent_id = %llu",
                     jobnet_id);
    if (result == NULL)
        return -1;
