...
//
/////////////////////////////////////////////////////////////////////////////////
static int jbxssh_waitsocket(jbxssh_t * obj)
{
    int rc;
    struct timeval tv;
    fd_set fd;
...
jbxssh_t *jbxssh_new(void)
{
    ...
    obj = (jbxssh_t *) malloc(sizeof(jbxssh_t));
...
int jbxssh_session(jbxssh_t * obj, const char *host, const int port)
{
    ...
    if (obj == NULL || host == NULL)
        return -1;
    jbxlog_debug("In %s() host: %s, port: %d", __func__, host, port);
    jbxssh_init(obj);
    ...
    jbxlog_debug("In %s() host: %s, port: %d, sock: %d", __func__, host,
                 port, obj->socket);
                             ...
                             const char *password)
{
    ...
    if (obj == NULL || user == NULL || password == NULL)
        return -1;
    jbxlog_debug("In %s() user: %s", __func__, user);
    if (obj->session == NULL) {
        jbxlog_error("In %s() ssh session is not established", __func__);
        return -1;
    }
    // ssh userauth
    while (1) {
                              ...
                              const char *passphrase)
{
    ...
    cfg = jbxconf_get();
    if (cfg == NULL) {
        jbxlog_error("In %s() conf is NULL", __func__);
        return -1;
    }
    // get tmp dir
    rv = APR_SUCCESS;
    ...
    if (rv != APR_SUCCESS) {
        apr_strerror(rv, errmsg, sizeof(errmsg));
        jbxlog_error("In %s() can not get temp dir. %s", __func__, errmsg);
        return -1;
    }
    // get tmp file
    privatefile =
    ...
    if (rv != APR_SUCCESS) {
        apr_strerror(rv, errmsg, sizeof(errmsg));
        jbxlog_error("In %s() can not mktemp file [%s]. %s", __func__,
                     privatefile, errmsg);
        return -1;
    }
    // save private key
    ...
    jbxlog_debug("In %s() create private key file. %s", __func__,
                 privatefile);
                ...
                const char *command)
{
    ...
    if (obj->channel == NULL) {
        jbxlog_error("In %s() ssh channel is not established", __func__);
        return -1;
    }
    ...
    while (1) {
        ...
        if (rc_ssh >= 0) {
            ...
            jbxlog_warn("In %s() libssh2_channel_read failed. rc_ssh: %d",
                        __func__, rc_ssh);
            return -1;
        }
    }
    jbxlog_debug("In %s() stdout: \n%s", __func__, obj_jobres->stdout);
    ...
    if (str_signal != NULL) {
        jbx_strncat(obj_jobres->exitsignal, str_signal, EXIT_SIGNAL_SIZE);
