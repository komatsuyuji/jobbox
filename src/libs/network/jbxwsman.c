...
jbxwsman_t *jbxwsman_new(void)
{
    ...
    obj = (jbxwsman_t *) malloc(sizeof(jbxwsman_t));
    obj->client = NULL;
...
int jbxwsman_init(jbxwsman_t * obj)
{
    ...
    memset(obj->shell_id, 0, JBX_WSMAN_SIZE);
    memset(obj->command_id, 0, JBX_WSMAN_SIZE);
    memset(obj->errmsg, 0, JBX_WSMAN_SIZE);
                 ...
                 const char *path, const char *auth)
{
    ...
    jbxlog_trace("In %s() host: %s", __func__, host);
...
WsXmlDocH jbxwsman_send(jbxwsman_t * obj, WsXmlDocH request)
{
    ...
    if (response == NULL) {
        snprintf(obj->errmsg, JBX_WSMAN_SIZE, "response is NULL.");
        goto error;
    }
    // trace response 
    buf = NULL;
    ...
    ws_xml_dump_memory_node_tree(ws_xml_get_doc_root(response), &buf,
                                 &size);
    jbxlog_debug("In %s() size: %d, response: \n%s", __func__, size, buf);
    jbx_free(buf);

    return response;
  error:
    jbxlog_warn("In %s() send request failed. endpoint: %s, error: %s",
                         ...
                         const char *shell_id, const char *message_id)
{
    ...
    if (envelope == NULL || header == NULL) {
        jbxlog_warn("In %s() soap xml is incorrect", __func__);
        return -1;
    }
    // get serctx 
                        ...
                        const int codepage, const char *idle_timeout)
{
    ...
    // create header data
    jbxwsman_soap_header(obj, rqstDoc, TRANSFER_ACTION_CREATE,
    ...
    node =
        ws_xml_add_child(rsp, XML_NS_RSP, MS_OUTPUT_STREAMS,
                         "stdout stderr");
    // send message
    respDoc = jbxwsman_send(obj, rqstDoc);
    if (respDoc == NULL) {
        jbxlog_warn("In %s() can not get response", __func__);
        goto error;
    }
    // get reponse data
    ...
    snprintf(obj->shell_id, JBX_WSMAN_SIZE, "%s",
             ws_xml_get_node_text(node));
    jbxlog_debug("In %s() shell_id: %s", __func__, obj->shell_id);
                         ...
                         const char *skip_cmd_shell)
{
    ...
    // create header data
    jbxwsman_soap_header(obj, rqstDoc, ACTION_COMMAND, RESOURCE_URI_CMD,
                         obj->shell_id, NULL);
    header = ws_xml_get_soap_header(rqstDoc);
    body = ws_xml_get_soap_body(rqstDoc);
    ...
    snprintf(obj->command_id, JBX_WSMAN_SIZE, "%s",
             ws_xml_get_node_text(node));
    jbxlog_debug("In %s() command_id: %s", __func__, obj->command_id);
                                    ...
                                    const int codepage)
{
    ...
    // create header data
    jbxwsman_soap_header(obj, rqstDoc, ACTION_RECEIVE, RESOURCE_URI_CMD,
                         obj->shell_id, NULL);
    header = ws_xml_get_soap_header(rqstDoc);
    body = ws_xml_get_soap_body(rqstDoc);
    ...
    for (i == 0; i < cnt; i++) {
        ...
        if (strcmp(attr_value, "stdout") == 0) {
            jbx_strncat(obj_jobres->stdout, buf, JOB_RESULT_SIZE);
        } else if (strcmp(attr_value, "stderr") == 0) {
            jbx_strncat(obj_jobres->stderr, buf, JOB_RESULT_SIZE);
        }
        jbx_free(buf);
    }
                                ...
                                const int codepage)
{
    ...
    while (obj->command_done == 0) {
        if (jbxwsman_raw_get_command_output(obj, obj_jobres, codepage) !=
            0)
            return -1;
    }
...
int jbxwsman_cleanup_command(jbxwsman_t * obj)
{
    ...
    // create header data
    generate_uuid(uuidBuf, sizeof(uuidBuf), 0);
    jbxwsman_soap_header(obj, rqstDoc, ACTION_SIGNAL, RESOURCE_URI_CMD,
...
int jbxwsman_close_shell(jbxwsman_t * obj)
{
    ...
    // create header data
    generate_uuid(uuidBuf, sizeof(uuidBuf), 0);
    jbxwsman_soap_header(obj, rqstDoc, ACTION_DELETE, RESOURCE_URI_CMD,
                         obj->shell_id, uuidBuf);
    body = ws_xml_get_soap_body(rqstDoc);
                  ...
                  const char *command, const int codepage)
{
    ...
    rc = -1;
    jbxlog_debug("In %s() command: %s", __func__, command);
    if (obj->client == NULL) {
        jbxlog_error("In %s() wsman is NULL", __func__);
        return -1;
    }
    // init jobresult
    ...
    // open shell
    if (jbxwsman_open_shell
        (obj, "stdin", "stdout stderr", NULL, NULL, "FALSE", codepage,
         NULL) != 0) {
        jbxlog_warn("In %s() can not open shell", __func__);
        return -1;
    }
    // run command
    if (jbxwsman_run_command(obj, command, "TRUE", "FALSE") != 0)
        return -1;

    // get command output
    if (jbxwsman_get_command_output(obj, obj_jobres, codepage) != 0)
        return -1;

    // cleanup command
    if (jbxwsman_cleanup_command(obj) != 0) {
        jbxlog_warn("In %s() can not cleanup command", __func__);
    } else {
        if (jbxwsman_close_shell(obj) != 0) {
            jbxlog_warn("In %s() can not close shell", __func__);
        }
    }

    // reset exitcode and exitsignal again
    jbxlog_debug("In %s() exitcode: %d, exitsignal: %s", __func__,
