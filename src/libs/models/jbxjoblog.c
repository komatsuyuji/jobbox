...
/////////////////////////////////////////////////////////////////////////////////
static jbxjoblog_t *jx_joblog = NULL;
...
int jbxjoblog_create(char *joblogfile)
{
    ...
    jx_joblog = (jbxjoblog_t *) malloc(sizeof(jbxjoblog_t));
...
void jbxjoblog_destroy(void)
{
    ...
    jbx_free(jx_joblog->joblogfile);
    jbx_free(jx_joblog->logfile);
...
int jbxjoblog_open(void)
{
    ...
    sprintf(jx_joblog->today, "%s", jbx_today());
    ...
    if (s != APR_SUCCESS) {
        ...
        jbxlog_error("can not open the file [%s]. %s\n",
...
int jbxjoblog_write(const char *message)
{
    ...
    // rotate the file
    if (strcmp(jbx_today(), jx_joblog->today) != 0
        || jbxjoblog_filechk() != 0) {
        jbxjoblog_close();
        if (jbxjoblog_open() != 0) {
            return -1;
    ...
    if (s != APR_SUCCESS) {
        apr_strerror(s, errmsg, sizeof(errmsg));
        jbxlog_error("can not write to the file [%s]. %s\n",
                     jx_joblog->logfile, errmsg);
        goto error;
    }
...
int jbxjoblog_jobunit(jobunit_t * obj)
{
    ...
    csv_description = jbx_csv(obj->description);
    snprintf(message, sizeof(message), "%s,%llu,%s,%s,%s,%s,%s,%s\n",
             jbx_timestamp(), obj->id, obj->path, obj->name,
             csv_description, jbxjoblog_kind(obj->kind),
             jbxjoblog_status(obj->status),
             jbxjoblog_timeout_occur(obj->timeout_occur));

    jbx_free(csv_description);
    return jbxjoblog_write(message);
}
