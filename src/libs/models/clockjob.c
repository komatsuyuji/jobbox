...
int clockjob_load(clockjob_t * obj, dbi_result res)
{
    ...
    obj->predetermined_time =
        jbxdb_get_datetime(res, "predetermined_time");
...
int clockjob_load_one(clockjob_t * obj, dbi_result res)
{
    ...
    if (num != 1) {
        jbxlog_warn("In %s() can not find clockjob. numrows: %llu",
                    __func__, num);
        return 1;
    }
...
int clockjob_select(clockjob_t * obj, const apr_uint64_t jobunit_id)
{
    ...
    result =
        jbxdb_select("SELECT * FROM clockjobs WHERE jobunit_id = %llu",
                     jobunit_id);
    if (result == NULL)
        return -1;
...
apr_uint64_t proc_clockjob_insert(clockjob_t * obj)
{
    ...
    predetermined_time = jbxdb_escape_datetime(obj->predetermined_time);
...
int clockjob_execute(jobunit_t * proc_jobunit)
{
    ...
    case CLOCKJOB_BASETIME_JOB_STARTDAY:
        basetime = jbx_midnight(time(NULL));
        break;
    ...
    case CLOCKJOB_BASETIME_JOBNET_STARTDAY:
        basetime =
            jbx_midnight(proc_jobunit_get_start_time
    ...
    case CLOCKJOB_BASETIME_ROOTJOBNET_STARTDAY:
        basetime =
            jbx_midnight(proc_jobunit_get_rootjobnet_starttime
                         (proc_jobunit->parent_id));
        break;
    ...
    predetermined_time = jbxdb_escape_datetime(mktime(tm));
    jbxlog_debug
...
int clockjob_monitor(jobunit_t * proc_jobunit)
{
    ...
    jbxlog_trace("In %s() id: %llu, status: %d, force_stop: %d", __func__,
