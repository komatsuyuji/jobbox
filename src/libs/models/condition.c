...
int condition_load(condition_t * obj, dbi_result res)
{
    ...
    obj->cond = jbxdb_get_string(res, "cond");
...
apr_uint64_t proc_condition_insert(condition_t * obj)
{
    ...
    // escape strinq
    esc_cond = jbxdb_escape_string(obj->cond);
                           ...
                           const apr_uint64_t proc_jobunit_id)
{
    ...
    result =
        jbxdb_select("SELECT * FROM conditions WHERE jobunit_id = %llu",
                     jobunit_id);
    if (result == NULL)
        return -1;
...
int condition_execute(condition_t * obj, jobresult_t * obj_res)
{
    ...
    jbxlog_debug("In %s() id: %llu", __func__, obj->id);

    switch (obj->kind) {
    case CONDITION_KIND_EXITCODE:
        rv = jbx_on_range(obj_res->exitcode, obj->cond);
        break;
    case CONDITION_KIND_EXITSIGNAL:
        rv = jbx_on_regexp(obj_res->exitsignal, obj->cond);
        break;
    case CONDITION_KIND_STDOUT:
        rv = jbx_on_regexp(obj_res->stdout, obj->cond);
        break;
    case CONDITION_KIND_STDERR:
        rv = jbx_on_regexp(obj_res->stderr, obj->cond);
        break;
    default:
        jbxlog_error("In %s() unknown condition kind. %d",
                     __func__, obj->kind);
        return JOBBOX_ERROR;
    }

    if (rv == JOBBOX_TRUE) {
        rv = (obj->negative) ? JOBBOX_FALSE : JOBBOX_TRUE;
    } else if (rv == JOBBOX_FALSE) {
        rv = (obj->negative) ? JOBBOX_TRUE : JOBBOX_FALSE;
    }
                       ...
                       jobresult_t * obj_res)
{
    ...
    while (dbi_result_next_row(result)) {
        ...
        if (rv_error == JOBBOX_FALSE)
            continue;

        if (rv_error == JOBBOX_TRUE) {
    ...
    do {
        ...
        if (rv_branch == JOBBOX_FALSE)
            continue;

        if (rv_branch == JOBBOX_TRUE) {
            execlog_info(obj->proc_jobunit_id,
