...
int connectors_create(connectors_t * obj, const int num)
{
    ...
    jbxlog_trace("In %s() num: %d", __func__, num);
    if (obj == NULL)
        return -1;
...
int connectors_select(connectors_t * obj, const apr_uint64_t jobunit_id)
{
    ...
    result =
        jbxdb_select("SELECT * FROM connectors WHERE jobunit_id = %llu",
                     jobunit_id);
    if (result == NULL)
        return -1;
...
int proc_connectors_insert(connectors_t * obj)
{
    ...
    jbxlog_debug("In %s() num: %d", __func__, obj->num);
...
int connectors_calc_boot(connectors_t * obj, const apr_uint64_t next_jobid)
{
    ...
    jbxlog_trace("In %s() next_jobid: %llu", __func__, next_jobid);
    ...
    jbxlog_debug("In %s() next_jobid: %llu, boot: %d", __func__,
                 next_jobid, boot);
