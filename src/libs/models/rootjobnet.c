...
int rootjobnet_load_one(rootjobnet_t * obj, dbi_result res)
{
    ...
    if (num != 1) {
        jbxlog_warn("In %s() can not find rootjobnet. numrows: %llu",
                    __func__, num);
        return 1;
    }
...
int rootjobnet_select(rootjobnet_t * obj, const apr_uint64_t jobunit_id)
{
    ...
    jbxlog_trace("In %s() jobunit_id: %llu", __func__, jobunit_id);
    result =
        jbxdb_select
...
apr_uint64_t proc_rootjobnet_insert(rootjobnet_t * obj)
{
    ...
    active_since = jbxdb_escape_datetime(obj->active_since);
    active_till = jbxdb_escape_datetime(obj->active_till);
 ...
 VALUES(%llu, %llu, %d, %s, %s, %llu, %d, %d, %d)",
    ...
    jbx_free(active_since);
    jbx_free(active_till);
    return id;
}
...
int rootjobnet_on_active(rootjobnet_t * obj, const time_t t)
{
    ...
    target_date = jbx_time2day(t);
    since_date = jbx_time2day(obj->active_since);
    till_date = jbx_time2day(obj->active_till);

    // check active date 
    jbxlog_trace("In %s() target_date: %d, since_date: %d, till_date: %d",
                 __func__, target_date, since_date, till_date);
    if (target_date >= since_date) {
        if (target_date <= till_date || obj->active_till <= 0) {
            jbxlog_trace("In %s() the rootjobnet is active. id: %llu",
                         __func__, obj->id);
            return JOBBOX_TRUE;
        } else {
            jbxlog_trace("In %s() the rootjobnet is not active. id: %llu",
                         __func__, obj->id);
            return JOBBOX_FALSE;
        }
    } else {
