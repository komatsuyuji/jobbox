...
apr_uint64_t proc_jobresult_insert(jobresult_t * obj)
{
    ...
    // escape strinq
    esc_exitsignal = jbxdb_escape_string(obj->exitsignal);
    esc_stdout = jbxdb_escape_string(obj->stdout);
    esc_stderr = jbxdb_escape_string(obj->stderr);
...
apr_uint64_t hist_jobresult_insert(jobresult_t * obj)
{
    ...
    created_at = jbxdb_escape_datetime(obj->created_at);
    updated_at = jbxdb_escape_datetime(obj->updated_at);
...
int jobresult_execute(jobresult_t * obj)
{
    ...
    // exitsignal
    if (obj->exitcode < 0) {
        jbxlog_warn("In %s() get a exitsignal: %s, proc_jobunit_id: %llu",
