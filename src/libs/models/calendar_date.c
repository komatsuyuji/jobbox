...
int calendar_date_load(calendar_date_t * obj, dbi_result res)
{
    ...
    obj->caledate = jbxdb_get_datetime(res, "caledate");
                         ...
                         const time_t cale_time)
{
    ...
    jbxlog_trace("In %s() calendar_id: %llu, cale_time: %ld",
                 __func__, calendar_id, cale_time);
    if (cale_time == 0) {
        jbxlog_warn("In %s() cale_time is unset", __func__);
        return JOBBOX_ERROR;
    }

    if (calendar_id == 0) {
        jbxlog_trace("In %s() not use calendar", __func__);
        return JOBBOX_TRUE;
    }
    ...
    jbxlog_trace("In %s() caledate: %s", __func__, caledate);
