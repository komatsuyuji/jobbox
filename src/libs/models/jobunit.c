...
int jobunit_load(jobunit_t * obj, dbi_result res)
{
    ...
    obj->start_time = jbxdb_get_datetime(res, "start_time");
    obj->end_time = jbxdb_get_datetime(res, "end_time");
    ...
    obj->schedule_time = jbxdb_get_datetime(res, "schedule_time");
...
int jobunit_load_one(jobunit_t * obj, dbi_result res)
{
    ...
    if (num != 1) {
        jbxlog_warn("In %s() can not find jobunit. numrows: %llu",
                    __func__, num);
        return 1;
    }
...
int jobunit_select(jobunit_t * obj, const apr_uint64_t id)
{
    ...
    jbxlog_trace("In %s() id: %llu", __func__, id);

    result = jbxdb_select("SELECT * FROM jobunits WHERE id = %llu", id);
    if (result == NULL)
        return -1;
...
apr_uint64_t proc_topjobnet_insert(jobunit_t * obj)
{
    ...
    esc_description = jbxdb_escape_string(obj->description);
    ts = jbx_time();
    schedule_time = jbxdb_escape_datetime(obj->schedule_time);
    ...
    id = jbxdb_insert("INSERT INTO proc_topjobnets(name, description, kind, hold, skip, timeout, created_at, updated_at, \
...
VALUES ('%s', '%s', %d, %d, %d, %d, '%s', '%s', %llu, %d, %d, %s, %d, %d)",
  ...
  error:
    jbx_free(schedule_time);
    jbx_free(esc_name);
    jbx_free(esc_description);
    return id;
}
...
apr_uint64_t hist_jobunit_insert(jobunit_t * obj)
{
    ...
    ts = jbx_time();
    schedule_time = jbxdb_escape_datetime(obj->schedule_time);
    start_time = jbxdb_escape_datetime(obj->start_time);
    end_time = jbxdb_escape_datetime(obj->end_time);
...
int proc_topjobnet_delete(const apr_uint64_t id)
{
    ...
    return jbxdb_execute("DELETE FROM proc_topjobnets WHERE id = %llu",
                         id);
}
...
int proc_jobunit_delete(const apr_uint64_t id)
{
    ...
    return jbxdb_execute("DELETE FROM proc_jobunits WHERE id = %llu", id);
}
...
int hist_jobunit_delete(const apr_uint64_t id)
{
    ...
    return jbxdb_execute("DELETE FROM hist_jobunits WHERE id = %llu", id);
}
...
int proc_jobunit_delete_parent_id(const apr_uint64_t parent_id)
{
    ...
    return
        jbxdb_execute("DELETE FROM proc_jobunits WHERE parent_id = %llu",
                      parent_id);
}
                     ...
                     const int parent)
{
    ...
    jbxlog_trace("In %s() id: %llu, size: %d, parent: %d", __func__, id, s,
                 parent);
    if (path == NULL || s <= 0)
        return -1;
...
time_t proc_jobunit_get_start_time(const apr_uint64_t id)
{
    ...
    while (dbi_result_next_row(result)) {
        start_time = jbxdb_get_datetime(result, "start_time");
        break;
    }
...
int proc_jobunit_timeout_occur(jobunit_t * obj)
{
    ...
    jbxlog_trace("In %s() id: %llu", __func__, obj->id);
...
int jobunit_flowing(jobunit_t * obj, const int kind)
{
    ...
    jbxlog_trace("In %s() id: %llu, kind: %d", __func__, obj->id, kind);
    ...
    while (dbi_result_next_row(result)) {
        ...
        rc_db =
            ...
            ("UPDATE proc_jobunits SET status = %d, finish = %d, updated_at = '%s' WHERE id = %llu",
             obj_next_job->status, obj_next_job->finish, jbx_time(),
             obj_next_job->id);
        if (rc_db != 0)
            goto error;
        num++;
