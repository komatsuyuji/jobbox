                          ...
                          const apr_uint64_t proc_jobunit_id)
{
    ...
    result =
        jbxdb_select("SELECT * FROM dateconds WHERE jobunit_id = %llu",
                     jobunit_id);
    if (result == NULL)
        return -1;
                     ...
                     const int beginning_of_week)
{
    ...
    if (obj == NULL)
        return JOBBOX_ERROR;
    jbxlog_trace("In %s() base_time: %ld, beginning_of_week: %d", __func__,
    ...
    jbxlog_debug("In %s() id: %llu, week_no: %d", __func__, obj->id,
                 week_no);
    ...
    case DATECOND_KIND_CALENDAR:
        ...
        break;
    default:
        jbxlog_error("In %s() unknown date condition kind. %d",
                     __func__, obj->kind);
        rv = JOBBOX_ERROR;
    }
