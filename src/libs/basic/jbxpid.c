...
//
/////////////////////////////////////////////////////////////////////////////////
int jbxpid_create(const char *pidfile)
{
    apr_status_t s;
    char errmsg[100];
    ...
    if (s != APR_SUCCESS) {
        ...
        jbxlog_error("In %s() can not open the file [%s]. %s", __func__,
                     pidfile, errmsg);
        return -1;
    }
    // lock pidfile
    ...
    if (s != APR_SUCCESS) {
        apr_strerror(s, errmsg, sizeof(errmsg));
        jbxlog_error("In %s() can not lock the file [%s]. %s", __func__,
                     pidfile, errmsg);
        return -1;
    }
    // write pid to pidile
    if (apr_file_printf(jx_pidfile_fp, "%d", getpid()) <= 0) {
        apr_strerror(s, errmsg, sizeof(errmsg));
        jbxlog_error("In %s() can not write to the file [%s]. %s",
