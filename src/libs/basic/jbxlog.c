...
int jbxlog_create(jbxconf_t * cfg)
{
    ...
    jx_log = (jbxlog_t *) malloc(sizeof(jbxlog_t));
...
int jbxlog_append(const int level, const char *fmt, va_list va)
{
    ...
    snprintf(message, sizeof(message), "%s [%d] [%5s] %s\n",
             jbx_timestamp(), getpid(), level_string, buf);
    if (jbxlog_write(message) != 0)
        return -1;
...
int jbxlog_trace(const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    rc = jbxlog_append(LOG_LEVEL_TRACE, fmt, va);
    va_end(va);
...
int jbxlog_debug(const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    rc = jbxlog_append(LOG_LEVEL_DEBUG, fmt, va);
    va_end(va);
...
int jbxlog_info(const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    rc = jbxlog_append(LOG_LEVEL_INFO, fmt, va);
    va_end(va);
...
int jbxlog_warn(const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    rc = jbxlog_append(LOG_LEVEL_WARN, fmt, va);
    va_end(va);
...
int jbxlog_error(const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    rc = jbxlog_append(LOG_LEVEL_ERROR, fmt, va);
    va_end(va);
...
int jbxlog_crit(const char *fmt, ...)
{
    ...
    va_start(va, fmt);
    rc = jbxlog_append(LOG_LEVEL_CRIT, fmt, va);
    va_end(va);
