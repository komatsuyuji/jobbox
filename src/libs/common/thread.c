...
//
/////////////////////////////////////////////////////////////////////////////////
static void jbx_signal_handler(int sig, siginfo_t * siginfo, void *context)
{
    int sipid = -1;
    int siuid = -1;
    int sicode = -1;

    jbxlog_debug("In %s() signal: %d, name: %s", __func__, sig,
                 apr_signal_description_get(sig));
    if (siginfo == NULL) {
        jbxlog_debug("In %s() siginfo: NULL", __func__);
    } else {
    ...
    if (context == NULL) {
        jbxlog_debug("In %s() context: NULL", __func__);
    }
    ...
    case SIGSEGV:
    case SIGBUS:
        jbxlog_crit("In %s() got signal[sig: %d, name: %s]", __func__, sig,
                    apr_signal_description_get(sig));
        exit(-1);
        break;
    case SIGINT:
...
//
/////////////////////////////////////////////////////////////////////////////////
void jbx_set_signal_handler(void)
{
    struct sigaction phan;

    jbxlog_debug("In %s()", __func__);
    phan.sa_sigaction = (void *) jbx_signal_handler;
